...
// --- LOCAL STORAGE KEY ---
const STORAGE_KEY = "soccer_team_manager_players";
...
// Player data model now includes a locally generated unique ID
interface Player {
...
// Availability status is local state, based on the Player data
...
/**
 * Loads player data from browser localStorage.
 * @returns An array of Player objects or an empty array if none found.
 */
const loadPlayersFromStorage = (): Player[] => {
  try {
    const storedData = localStorage.getItem(STORAGE_KEY);
    if (storedData) {
      // Parse the stored JSON string back into a Player array
      return JSON.parse(storedData) as Player[];
    }
  } catch (error) {
...
/**
 * Saves the current list of players to browser localStorage.
 * @param players The current list of players to save.
 */
const savePlayersToStorage = (players: Player[]) => {
  try {
    // Stringify the Player array before saving
...
export default function App() {
  // Initialize players state by reading from localStorage on component mount
  const [players, setPlayers] = useState<Player[]>(loadPlayersFromStorage());

  // Availability state is initialized from the loaded players
  const [availability, setAvailability] = useState<PlayerAvailability[]>(
    players.map((p) => ({ ...p, isAvailable: true }))
  );
  ...
  // 1. Initial Synchronization and Persistence Setup
  useEffect(() => {
    // This ensures availability state is correctly set up when players load from storage
    setAvailability(players.map((p) => ({ ...p, isAvailable: true })));
  }, [players]);

  // 2. Persistence Hook: Save players to localStorage whenever the players state changes
  useEffect(() => {
  ...
  // Function to add a new player (updates state, which triggers the persistence hook)
  const addPlayer = (playerData: Omit<Player, "id">) => {
    ...
    // The savePlayersToStorage hook will run automatically after this state update.
  };

  // Function to update availability (updates local state only)
  ...
  const generateBalancedTeams = () => {
    ...
    // 2. Greedy Assignment
    // Assign the highest skilled players alternately to the team with the lowest current skill score
    ...
    return (
      ...
      >
        ...
        <div className="flex justify-between items-center pt-2">
          ...
          >
            {isSubmitting ? "Registering..." : "Save Player"}
          </button>
          {showSuccess && (
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-8 font-sans">
      <header className="text-center mb-8">
        <h1 className="text-4xl font-extrabold text-gray-900 flex items-center justify-center">
          ...
          Balancer (Local Storage)
        </h1>
        <p className="text-gray-600 mt-2">
          Manage players, track availability, and generate fair teams.
        </p>
        <p className="text-xs text-green-600 mt-1 font-semibold">
          Player data is saved locally in your browser and will persist across
          sessions.
        </p>
      </header>
